...
int tknzr_cont_equal(tokenizer_t tknzr1, tokenizer_t tknzr2)
{
...
int tknzr_equal(tokenizer_t tknzr1, tokenizer_t tknzr2)
{
	return tknzr1.size == tknzr2.size && tknzr_cont_equal(tknzr1, tknzr2);
}

bool match_psh_psh_wrd(tokenizer_t *tknzr, size_t i)
{
...
void apply_psh_psh_wrd_opt(tokenizer_t *tknzr, tokenizer_t *tknzr2, size_t *i)
{
...
bool match_psh_psh_rdd(tokenizer_t *tknzr, size_t i)
{
	return tknzr->tokens[i].type == TKN_PSH &&
		   tknzr->tokens[i+1].type == TKN_PSH &&
...
void apply_psh_psh_rdd(tokenizer_t *tknzr, tokenizer_t *tknzr2, size_t *i)
{
	uint32_t addr = tknzr->tokens[*i].val.num[0] | (tknzr->tokens[*i+1].val.num[0] << 8);
...
bool match_psh_wrd_imm(tokenizer_t *tknzr, size_t i)
{
	return tknzr->tokens[i].type == TKN_PSH &&
		   tknzr->tokens[i+1].type == TKN_WRD_IMM;
}

void apply_psh_wrd_imm(tokenizer_t *tknzr, tokenizer_t *tknzr2, size_t *i)
{
...
bool match_rrd_rrd_wrd(tokenizer_t *tknzr, size_t i)
{
...
void apply_rrd_imm_rrd_imm_wrd(tokenizer_t *tknzr, tokenizer_t *tknzr2, size_t *i)
{
...
bool match_psh_rdd2_wrd(tokenizer_t *tknzr, size_t i)
{
	return tknzr->tokens[i].type == TKN_PSH &&
		   tknzr->tokens[i+1].type == TKN_RDD2_WRD;
}

void apply_psh_rdd2_wrd(tokenizer_t *tknzr, tokenizer_t *tknzr2, size_t *i)
{
...
bool match_rdd_inc_dup_wrd(tokenizer_t *tknzr, size_t i)
{
	return tknzr->tokens[i].type == TKN_RDD_IMM &&
...
void apply_rrd_inc_dup_wrd(tokenizer_t *tknzr, tokenizer_t *tknzr2, size_t *i)
{
	uint32_t addr1 = tknzr->tokens[*i].val.num[0];
...
int match_psh_psh_add(tokenizer_t *tknzr, size_t i)
{
	return tknzr->tokens[i].type == TKN_PSH &&
		   tknzr->tokens[i+1].type == TKN_PSH &&
...
void apply_psh_psh_add(tokenizer_t *tknzr, tokenizer_t *tknzr2, size_t *i)
{
...
int match_psh_add_imm(tokenizer_t *tknzr, size_t i)
{
	return tknzr->tokens[i].type == TKN_PSH &&
		   tknzr->tokens[i+1].type == TKN_ADD_IMM;
}

void apply_psh_add_imm(tokenizer_t *tknzr, tokenizer_t *tknzr2, size_t *i)
{
	uint32_t op_1 = tknzr->tokens[*i].val.num[0];
	uint32_t op_2 = tknzr->tokens[*i+1].val.num[0];
	token_t new_token = (token_t){.type = TKN_PSH, .val = {.num = {(uint8_t)(op_2 + op_1)}}};
...
int match_psh_psh_sub(tokenizer_t *tknzr, size_t i)
{
	return tknzr->tokens[i].type == TKN_PSH &&
		   tknzr->tokens[i+1].type == TKN_PSH &&
...
void apply_psh_psh_sub(tokenizer_t *tknzr, tokenizer_t *tknzr2, size_t *i)
{
	uint32_t op_1 = tknzr->tokens[*i].val.num[0];
	uint32_t op_2 = tknzr->tokens[*i+1].val.num[0];
...
int match_psh_sub_imm(tokenizer_t *tknzr, size_t i)
{
	return tknzr->tokens[i].type == TKN_PSH &&
		   tknzr->tokens[i+1].type == TKN_SUB_IMM;
}

void apply_psh_sub_imm(tokenizer_t *tknzr, tokenizer_t *tknzr2, size_t *i)
{
	uint32_t op_1 = tknzr->tokens[*i].val.num[0];
	uint32_t op_2 = tknzr->tokens[*i+1].val.num[0];
	token_t new_token = (token_t){.type = TKN_PSH, .val = {.num = {(uint8_t)(op_1 - op_2)}}};
...
int match_psh2_2wrd_imm(tokenizer_t *tknzr, size_t i)
{
...
void apply_psh2_2wrd_imm(tokenizer_t *tknzr, tokenizer_t *tknzr2, size_t *i)
{
	uint32_t op_1 = tknzr->tokens[*i].val.num[0];
...
void optimize(tokenizer_t *tknzr)
{
